...
public class IndexController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sl.isRole(regUser, "ADMIN")) {
            request.setAttribute("info", "Добро пожаловать " + regUser.getName() + "!");
            request.getRequestDispatcher(PageReturner.getPage("index_admin"))
                    .forward(request, response);
            return;
            
        } else if (sl.isRole(regUser, "USER")) {
            request.setAttribute("info", "Добро пожаловать " + regUser.getName() + "!");
            request.getRequestDispatcher(PageReturner.getPage("index_user"))
                    .forward(request, response);
            return;
            
        } else if (sl.isRole(regUser, "MASTER")) {
            request.setAttribute("info", "Добро пожаловать " + regUser.getName() + "!");
            request.getRequestDispatcher(PageReturner.getPage("index_master"))
                    .forward(request, response);
            return;
            
        } else if (sl.isRole(regUser, "WORKER")) {
            request.setAttribute("info", "Добро пожаловать " + regUser.getName() + "!");
            request.getRequestDispatcher(PageReturner.getPage("index_worker"))
                    .forward(request, response);
            return;
